if (game:GetService("CoreGui")):FindFirstChild("DivineHub") and (game:GetService("CoreGui")):FindFirstChild("ScreenGui") then
	(game:GetService("CoreGui")).DivineHub:Destroy();
...
OutlineButton.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
OutlineButton.Position = UDim2.new(0, 15, 0, 80)
OutlineButton.Size = UDim2.new(0, 33, 0, 34)
OutlineButton.ZIndex = 2147483647
CreateRounded(OutlineButton, 10)
...
ImageButton.MouseButton1Click:Connect(function()
    ...
    pulsando = true
    pulseStart = tick()
    local hub = game.CoreGui:FindFirstChild("DivineHub")
    if hub then
    ...
    while true do
        ...
        if pulsando then
            ...
            if t <= pulseDuration then
                local formula = math.sin((t / pulseDuration) * math.pi)
                OutlineButton.Size = UDim2.new(0, 33 + (formula * pulsePower), 0, 34 + (formula * pulsePower))
            else
                pulsando = false
                OutlineButton.Size = normalSize
            end
        end
        task.wait(0.01)
    end
...
ImageButton.MouseButton1Click:connect(function()
	(game.CoreGui:FindFirstChild("DivineHub")).Enabled = not (game.CoreGui:FindFirstChild("DivineHub")).Enabled;
end);
...
(getgenv()).LoadConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("DivineHub") then
			makefolder("DivineHub");
		end;
		if not isfolder("DivineHub/Library/") then
			makefolder("DivineHub/Library/");
		end;
		if not isfile(("DivineHub/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("DivineHub/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(SettingsLib));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("DivineHub/Library/" .. game.Players.LocalPlayer.Name .. ".json"));
...
(getgenv()).SaveConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfile(("DivineHub/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			...
			writefile("DivineHub/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
		end;
	else
		return warn("Status : Undetected Executor");
	end;
...
function Update:Window(Config)
	...
	local DivineHub = Instance.new("ScreenGui");
	DivineHub.Name = "DivineHub";
	DivineHub.Parent = game.CoreGui;
	DivineHub.DisplayOrder = 999;
	local OutlineMain = Instance.new("Frame");
	OutlineMain.Name = "OutlineMain";
	OutlineMain.Parent = DivineHub;
